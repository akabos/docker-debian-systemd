...
RUN sed -ri /etc/systemd/journald.conf -e 's!^#?ForwardToConsole=.*!ForwardToConsole=yes!'
RUN sed -ri /etc/systemd/system.conf -e 's!^#?LogColor=.*!LogColor=no!'

ADD container-boot.service /etc/systemd/system/container-boot.service
RUN mkdir -p /etc/container-boot.d && systemctl enable container-boot.service
